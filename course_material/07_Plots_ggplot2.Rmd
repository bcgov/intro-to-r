...
ggplot(data = veg, mapping = aes(x = plot.id, y = pc_cover)) +
  geom_point()

# this shows there is an error in the data ! There is a value with 510 %?

veg <- veg %>%
  mutate(pc_cover = ifelse(pc_cover == 510 , 100, pc_cover))
...
want to show on our figure, in this example the vegetation data we read in
earlier. For the second argument,  we passed in the `aes` function, which
tells `ggplot` how variables in the **data** map to *aesthetic* properties of
the figure, in this case the **x** and **y** locations. Here we told `ggplot` we
want to plot the "plot id" column of the veg data frame on the x-axis, and
the "percent cover" column on the y-axis. Notice that we didn't need to explicitly
pass `aes` these columns (e.g. `x = veg[, "pc_cover"]`), this is because
...
## Challenge 1
 ...
 Modify the example so that the figure shows how percent cover has
 changed per site id:

```{r, eval=FALSE}
 ggplot(data = veg, mapping = aes(x = plot.id, y = pc_cover) + geom_point()
        
```

Hint: the gapminder dataset has a column called "fid", which should appear
 on the x-axis.
...
ggplot(data = veg, mapping = aes(x = plot.id, y = pc_cover, colour = as.factor(fid))) +
   geom_line()
...
ggplot(data = veg, mapping = aes(x = fid, y = pc_cover, colour = as.factor(fid)), group = plot.id) +
   geom_line()
```
...
```{r ch1-sol}
ggplot(data = veg, mapping = aes(x = fid, y = pc_cover)) + geom_point()
```
...
```{r ch2-sol}
ggplot(data = veg, mapping = aes(x = fid, y = pc_cover, colour = as.factor(plot.id))) +
   geom_point()
